...
Pick **one** method. Prefer **Windows + Chrome/Edge headless** when Node is not available; otherwise use **Node + Playwright**.

#### Option A — Windows (no Node, no Playwright)

Use the bundled PowerShell script. It launches **Chrome or Edge** in headless mode and relies on Chromium’s built-in `--screenshot` and `--force-device-scale-factor` (no extra install).

```powershell
powershell -ExecutionPolicy Bypass -File {baseDir}/scripts/screenshot.ps1 C:\path\infographic.html C:\path\output.png 3 750
```

Optional **5th** argument: viewport height in CSS pixels (default `9000`). Increase if the image is **clipped at the bottom** (headless mode uses a fixed viewport height). See `{baseDir}/WINDOWS.md`.

From CMD: `scripts\screenshot.cmd` calls the same script.

#### Option B — Node + Playwright (macOS / Linux / Windows with Node)
...
**Screenshot (choose one path):**

- **Windows without Node:** PowerShell 5.1+ and **Google Chrome** or **Microsoft Edge** — use `scripts/screenshot.ps1` (see `WINDOWS.md`).
- **Node workflow:** **Node.js** (any recent version) + **Playwright** with Chromium (`npx playwright install chromium`) — use `scripts/screenshot.js`.

**Optional:** For Telegram lossless delivery: `curl` + bot token in `~/.openclaw/openclaw.json` (see `scripts/send_telegram.sh`; Windows users can use `curl.exe` similarly if available).
